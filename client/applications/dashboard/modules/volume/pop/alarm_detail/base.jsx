        ...
        if (res.length !== 0) {
          request.getMeasures(ids, granularity, timeUtils.getTime(time)).then((_r) => {
            let arr = _r.map((r, index) => ({
              ...
              yAxisData: utils.getChartData(r, granularity, timeUtils.getTime(time), metricType[index], 'volume'),
              xAxis: utils.getChartData(r, granularity, timeUtils.getTime(time), metricType[index])
            }));
            updateContents(arr);
          });
        } else {

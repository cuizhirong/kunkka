  ...
  render: function() {
    return (
      <form method="POST" onSubmit={this.doSubmit} ref="loginForm">
        ...
        <input name="email" id="id_email" type="text" autoFocus="autoFocus"
          placeholder="邮箱或用户名" className={'email' + (this.state.usernameError ? ' error' : '')}
          tabIndex="1" onChange={this.onEmailChange} value={this.state.username} />
        <span className="password-icon"></span><input name="password" id="id_password"
          placeholder="密码" type="password" className={'password' + (this.state.passwordError ? ' error' : '')}
          tabIndex="2" onChange={this.onPwdChange} value={this.state.password} />
        <div className="tip">
